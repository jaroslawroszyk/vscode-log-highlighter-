...
function activate(context) {
    ...
    storedHighlights = context.globalState.get('storedHighlights', []);
    const editor = vscode.window.activeTextEditor;
    if (editor) {
        applyHighlights(editor);
    }
    context.subscriptions.push(vscode.commands.registerCommand('extension.highlightSelection', () => {
...
async function handleAddHighlight(context, editor, ignoreCase, customColor) {
    if (!editor) {
    ...
    const exists = storedHighlights.some(h => h.word === selectedTextRaw && h.ignoreCase === ignoreCase);
    if (exists) {
    ...
    if (customColor) {
        const picked = await vscode.window.showQuickPick(popularColors.map(c => c.label).concat(['Custom color (hex)']), { placeHolder: 'Select a highlight color or pick "Custom color (hex)" to enter manually' });
        if (!picked) {
            return;
        }
...
async function handleRemoveHighlight(context, editor) {
    if (!editor) {
        vscode.window.showInformationMessage('No active editor');
        return;
    }
...
async function handleRemoveAllHighlights(context, editor) {
    for (const key in highlights) {
        highlights[key].dispose();
        delete highlights[key];
    }
